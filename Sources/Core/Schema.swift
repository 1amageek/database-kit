...
public final class Schema: Sendable {
    ...
    public init(
        ...
        let runtimePolymorphicGroups = Self.buildPolymorphicGroups(from: polymorphicMembers)
        self.polymorphicGroups = runtimePolymorphicGroups
        ...
        for descriptors in polymorphicIndexDescriptorsByIdentifier.values {
            allIndexDescriptors.append(contentsOf: descriptors)
        }
    ...
    public func indexDescriptors(for itemType: String) -> [IndexDescriptor] {
        guard let entity = entitiesByName[itemType] else {
            return polymorphicIndexDescriptorsByIdentifier[itemType] ?? []
        }
        return entity.indexDescriptors
    }

    private static func buildPolymorphicGroups(
        from membersByIdentifier: [String: [any Persistable.Type]]
    ) -> [PolymorphicGroup] {
        membersByIdentifier
            .compactMap { (identifier, memberTypes) -> PolymorphicGroup? in
                guard let first = memberTypes.first,
                      let protocolType = first as? any Polymorphable.Type else {
                    return nil
                }

                var indexes: [AnyIndexDescriptor] = []
                indexes.reserveCapacity(protocolType.polymorphicIndexDescriptors.count)
                for descriptor in protocolType.polymorphicIndexDescriptors {
                    indexes.append(AnyIndexDescriptor(descriptor))
                }

                var memberTypeNames: [String] = []
                memberTypeNames.reserveCapacity(memberTypes.count)
                for memberType in memberTypes {
                    memberTypeNames.append(memberType.persistableType)
                }
                return PolymorphicGroup(
                    identifier: identifier,
                    directoryComponents: PolymorphicGroup.extractDirectoryComponents(
                        from: protocolType.polymorphicDirectoryPathComponents
                    ),
                    directoryLayer: protocolType.polymorphicDirectoryLayer,
                    indexes: indexes,
                    memberTypeNames: memberTypeNames.sorted()
                )
            }
            .sorted { $0.identifier < $1.identifier }
    }

    private static func buildPolymorphicIndexDescriptors(
        from membersByIdentifier: [String: [any Persistable.Type]]
    ) -> [String: [IndexDescriptor]] {
        Dictionary(
