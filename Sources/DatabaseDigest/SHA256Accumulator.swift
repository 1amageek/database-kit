...
public struct SHA256Accumulator: Sendable {
    ...
    public mutating func update(_ source: UnsafeRawBufferPointer) {
        let sourceCount = UInt64(source.count)
        let (nextTotal, overflow) = totalByteCount.addingReportingOverflow(
            sourceCount
        )
        precondition(!overflow, "SHA-256 input length exceeds UInt64")
        ...
        while source.count - sourceOffset >= 64 {
            ...
            state = Self.compressed(block, state: state)
            sourceOffset += 64
        }
    ...
    public consuming func withUnsafeDigestBytes<Result>(
        ...
        let bitCount = totalByteCount.multipliedReportingOverflow(by: 8)
        precondition(!bitCount.overflow, "SHA-256 bit length exceeds UInt64")
        ...
        for offset in 0..<8 {
            pending[56 + offset] = UInt8(
                truncatingIfNeeded: bitCount.partialValue
                    >> UInt64((7 - offset) * 8)
            )
        }
        processPendingBlock()
    ...
    private mutating func processPendingBlock() {
        ...
        let nextState = withUnsafeBytes(of: &pending) { block in
            Self.compressed(block, state: currentState)
        }
        state = nextState
    }
